...
class MeshManager: NSObject, ObservableObject {
    ...
    func startScanning() {
        ...
        scanTimeoutTask = Task { [weak self] in
            do {
                ...
                // Only mark timeout if still scanning and no nodes found
    ...
    private func saveLocalData() {
        ...
        do {
            let dataToSave = MeshData(
                ...
                messages: currentMessages // Save messages to local storage on macOS
            )
    ...
    private func loadLocalData() {
        do {
            ...
            self.messagesList = decoded.messages // Restore message history if any exists (empty in new runs)
...
extension MeshManager: CBCentralManagerDelegate {
    ...
    nonisolated func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        Task { @MainActor in
            ...
            // MTU negotiation completes during service discovery, so we query maxWriteLength
            // in didDiscoverCharacteristicsFor after the exchange.
