...
static void scheduleReboot(uint32_t ms) {
    TimerHandle_t t =
        xTimerCreate("reboot", pdMS_TO_TICKS(ms), pdFALSE, nullptr, rebootNow);
    if (t)
        xTimerStart(t, 0);
}
...
void setupOTA(AsyncWebServer &server, painlessMesh &mesh) {
    ...
    server.on("/update", HTTP_GET, [](AsyncWebServerRequest *r) {
        ...
        if (LittleFS.exists("/html_gz/update.html.gz")) {
            auto res = r->beginResponse(LittleFS, "/html_gz/update.html.gz",
                                        "text/html");
            res->addHeader("Content-Encoding", "gzip");
            res->addHeader("Cache-Control", "no-cache");
            r->send(res);
        } else if (LittleFS.exists("/template/update.html")) {
            auto res = r->beginResponse(LittleFS, "/template/update.html",
                                        "text/html");
            res->addHeader("Cache-Control", "no-cache");
            r->send(res);
        } else {
            r->send(404, "text/plain",
                    "update.html not found — run build.sh then pio run -t "
                    "uploadfs");
        }
    });
    ...
    server.on("/ota/status", HTTP_GET, [](AsyncWebServerRequest *r) {
        static const char *NAMES[] = {"idle",     "uploading", "distributing",
                                      "flashing", "done",      "error"};
        ...
        j += "\",";
        j += "\"uploadPct\":";
        j += g_otaStatus.uploadPct;
        j += ",";
        j += "\"sentChunks\":";
        j += g_otaStatus.sentChunks;
        j += ",";
        j += "\"totalChunks\":";
        j += g_otaStatus.totalChunks;
        j += ",";
        j += "\"ackedNodes\":";
        j += g_otaStatus.ackedNodes;
        j += ",";
        j += "\"peerCount\":";
        j += g_otaStatus.peerCount;
        j += ",";
        j += "\"errMsg\":\"";
        j += g_otaStatus.errMsg;
        j += "\"}";
    ...
    server.on("/ota/setpw", HTTP_GET, [](AsyncWebServerRequest *r) {
        ...
        r->send(200, "application/json",
                "{\"ok\":true,\"note\":\"recompile with new OTA_PASS or use "
                "NVS\"}");
    });
    ...
    server.on(
        ...
        [](AsyncWebServerRequest *r) {
            ...
            if (s_targetAll) {
                ...
                md5b.begin();
                {
                    ...
                    uint8_t tmp[256];
                    while (f.available()) {
                        size_t n = f.read(tmp, sizeof(tmp));
                        md5b.add(tmp, n);
                    }
                    f.close();
                }
        ...
        [](AsyncWebServerRequest *r, String /*fn*/, size_t index, uint8_t *data,
           size_t len, bool final) {
            if (index == 0) {
                ...
                if (s_targetAll) {
                    ...
                    if (!s_uploadFile) {
                        Serial.println(
                            "[OTA] Failed to open ota.bin for writing");
                        s_authOK = false;
                        return;
                    }
...
void loopOTA() {
    ...
    if (now - s_announceAt < OTA_ANNOUNCE_MS)
        return;
    ...
    if (s_chunkSeq >= g_otaStatus.totalChunks) {
        ...
        if (allAcked || timedOut) {
            ...
            if (!Update.begin(fsz, U_FLASH)) {
                f.close();
                snprintf(g_otaStatus.errMsg, sizeof(g_otaStatus.errMsg), "%s",
                         Update.errorString());
                g_otaStatus.state = OTA_ERROR;
                return;
            }
    ...
    if (now - s_chunkTimer < OTA_CHUNK_MS)
        return;
    s_chunkTimer = now;
    ...
    if (n > 0) {
        String pkt = String(OTA_PREFIX) + "CHUNK|" + s_chunkSeq + "|" +
                     b64EncLocal(buf, n);
...
bool handleOTAMessage(const String &msg, uint32_t fromId) {
    ...
    if (body.startsWith("ANNOUNCE|")) {
        ...
        int chunks = rest.substring(p2 + 1).toInt();
    ...
    if (body.startsWith("CHUNK|")) {
        if (g_otaStatus.state == OTA_DISTRIBUTING)
            return true; // sender ignores own
        if (!Update.isRunning())
            return true;
        ...
        // Decode base64
        auto b64DecLocal = [](const String &in, uint8_t *out,
                              size_t outLen) -> int {
            const char *LUT = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuv"
                              "wxyz0123456789+/";
            ...
            for (size_t i = 0; i < len; i += 4) {
                int a = val(in[i]), b = val(in[i + 1]), c = val(in[i + 2]),
                    d = val(in[i + 3]);
                if (a < 0 || b < 0)
                    return -1;
    ...
    if (body.startsWith("END|")) {
        ...
        if (!Update.end(true)) {
            ...
            if (s_mesh) {
                String ack =
                    String(OTA_PREFIX) + "ACK|" + String(s_mesh->getNodeId());
